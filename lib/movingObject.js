(function() {
  ...
  MO.prototype = {
    ...
    outOfBounds: function(game, resp) {
      if (this.isCollidedWith({poly: game.LEFT_EDGE}, resp)) {
        return true;
      } else {
        resp.clear();
      }
      if (this.isCollidedWith({poly: game.RIGHT_EDGE}, resp)) {
        return true;
      } else {
        return this.game.outOfBounds(this.poly);
      }
    },
